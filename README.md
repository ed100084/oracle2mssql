...
| EHRPHRAFUNC_PKG | 52 | 34 | 18 | **65%** 🔧 |
| **合計** | **94** | **76** | **18** | **80.9%** |

> Session 10 完成 EHRPHRA12_PKG 32/32 全通過，EHRPHRAFUNC_PKG 進行中（34/52）。

### EHRPHRAFUNC_PKG 剩餘 18 個失敗（AST 引擎，Session 10 進行中）

| 類別 | 失敗 SP | 問題 |
|---|---|---|
| ROUND(x) nested | `f_count_time`, `hrasend_mail_IT` | `ROUND(expr_with_parens)` regex 未匹配 |
| MOD nested | `hrasend_mail2/abroad/abroadDoc` | `MOD(SUM(...), n)` regex 未匹配 |
| CAST mask | `f_getseniority_month/year`, `f_getyearenday` | `CAST(SUBSTRING(...), mask AS type)` |
| EXEC in function | `f_getvactime` | function 內 EXEC（12 處） |
| utl_smtp/utl_raw | `POST_HTML_MAIL×3` | Oracle SMTP API 無 T-SQL 對應 |
| @subject column | `POST_MIS_MSG`, `POST_MISMSG_MAIL` | parameter 名稱誤用為 INSERT 欄位名 |
| ORDER BY in subquery | `hrasend_mail_EF` | 子查詢含 ORDER BY（1033） |
| extra END | `f_FreesignTime` | 結構問題 |
| misc | `CheckMorning_mail`, `hrasend_mail_immi2` | 各異 |
...
| **AST**（方案 D）| `--engine ast` | EHRPHRA3_PKG 10/10 PASS，持續擴展中 | 新 Package / 複雜語法 |
...
### 立即（EHRPHRAFUNC_PKG 剩餘 18 個失敗，Session 10 進行中）

- [ ] `ROUND(expr_with_parens)` → `ROUND(expr, 0)`：改用 balanced-paren scanner
- [ ] `MOD(SUM(...), n)` → `(SUM(...) % n)`：改用 balanced-paren scanner
- [ ] `CAST(SUBSTRING(...), mask AS type)` → `CAST(SUBSTRING(...) AS type)`：改用 lazy regex
- [ ] `f_getvactime` EXEC in expression → function call 還原
- [ ] `POST_HTML_MAIL×3` utl_smtp/utl_raw 全面 TODO 注解
- [ ] `POST_MIS_MSG/MISMSG_MAIL` @subject 誤用為 INSERT 欄位名
- [ ] `hrasend_mail_EF` ORDER BY in subquery（1033）
- [ ] `f_FreesignTime` extra END
- [ ] `CheckMorning_mail`、`hrasend_mail_immi2` 個案修復

### 短期

- [ ] EHRPHRAFUNC_PKG 達成 ≥45/52（87%）
- [ ] 執行 `ora2mssql deploy --dry-run` 全包驗證
- [ ] 將三個 Package 部署至 MSSQL 開發庫

### AST 引擎後續

- [x] ~~執行 sp-check 驗證 EHRPHRA3_PKG AST 轉換結果~~ → **10/10 PASS（Session 9）**
- [x] ~~擴展 AST 引擎至 EHRPHRA12_PKG~~ → **32/32 PASS（Session 10）** ✅
- [ ] EHRPHRAFUNC_PKG 達成目標通過率（進行中，34/52）
- [ ] 支援 `%ROWTYPE` 欄位自動展開

---

*最後更新：2026-04-08（Session 10）*

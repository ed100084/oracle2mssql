...
**Deploy dry-run 狀態**：已加入 `_convert_cursor_syntax`, `_convert_select_into`, `_strip_named_params`, FOR loop 轉換等修復，尚待完整 sp-check 驗證。
...
| **AST**（方案 D）| `--engine ast` | POC 完成，持續改進中 | 新 Package / 複雜語法 |
...
- [ ] 執行 sp-check 驗證 EHRPHRA3_PKG AST 轉換結果
- [ ] 修復剩餘 sp-check 失敗項目（預期：游標名稱 @ 前綴、SELECT INTO 多欄位、named params）
- [ ] 支援 %ROWTYPE 欄位自動展開（比 Regex 引擎更可行，因有完整 Parse Tree）
- [ ] 將其餘 Package 納入 AST 引擎轉換

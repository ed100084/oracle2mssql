...
> Session 9 完成 EHRPHRA3_PKG AST 引擎的 sp-check 全數通過（10/10）。
...
- [x] ~~執行 sp-check 驗證 EHRPHRA3_PKG AST 轉換結果~~ → **10/10 PASS（Session 9 完成）**
- [ ] 擴展 AST 引擎至 EHRPHRA12_PKG（33 個 Routine）
- [ ] 擴展 AST 引擎至 EHRPHRAFUNC_PKG（53 個 Routine，含 4 個 Regex 引擎無法解決的失敗）
- [ ] 支援 `%ROWTYPE` 欄位自動展開（比 Regex 引擎更可行，因有完整 Parse Tree）

---

*最後更新：2026-04-08（Session 9）*

...
USE MIS
GO
IF  EXISTS (SELECT * FROM sys.objects so JOIN sys.schemas sc ON so.schema_id = sc.schema_id WHERE so.name = N'EHRPHRAFUNC_PKG$POST_ORIGIN_HTML_MAIL'  AND sc.name=N'hrp'  AND type in (N'P',N'PC'))
 DROP PROCEDURE [hrp].[EHRPHRAFUNC_PKG$POST_ORIGIN_HTML_MAIL]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE HRP.EHRPHRAFUNC_PKG$POST_ORIGIN_HTML_MAIL  
   @SENDER varchar(max),
...
BEGIN TRY
    EXEC sp_addextendedproperty
        N'MS_SSMA_SOURCE', N'HRP.EHRPHRAFUNC_PKG.POST_ORIGIN_HTML_MAIL',
        N'SCHEMA', N'hrp',
        N'PROCEDURE', N'EHRPHRAFUNC_PKG$POST_ORIGIN_HTML_MAIL'
END TRY
BEGIN CATCH

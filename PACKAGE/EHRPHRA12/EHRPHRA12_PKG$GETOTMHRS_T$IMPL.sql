...
USE MIS
GO
IF  EXISTS (SELECT * FROM sys.objects so JOIN sys.schemas sc ON so.schema_id = sc.schema_id WHERE so.name = N'EHRPHRA12_PKG$GETOTMHRS_T$IMPL'  AND sc.name=N'hrp'  AND type in (N'P',N'PC'))
 DROP PROCEDURE [hrp].[EHRPHRA12_PKG$GETOTMHRS_T$IMPL]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE HRP.EHRPHRA12_PKG$GETOTMHRS_T$IMPL  
...
BEGIN TRY
    EXEC sp_addextendedproperty
        N'MS_SSMA_SOURCE', N'HRP.EHRPHRA12_PKG.getOtmhrs_T',
        N'SCHEMA', N'hrp',
        N'PROCEDURE', N'EHRPHRA12_PKG$GETOTMHRS_T$IMPL'
END TRY
BEGIN CATCH

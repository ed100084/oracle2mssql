...
USE MIS
GO
IF  EXISTS (SELECT * FROM sys.objects so JOIN sys.schemas sc ON so.schema_id = sc.schema_id WHERE so.name = N'EHRPHRA12_PKG$HRAC010A_ADD'  AND sc.name=N'hrp'  AND type in (N'P',N'PC'))
 DROP PROCEDURE [hrp].[EHRPHRA12_PKG$HRAC010A_ADD]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE HRP.EHRPHRA12_PKG$HRAC010A_ADD  
   ...
   BEGIN
      ...
      DECLARE
         ...
         */
         ...
         @STOTADDHRS float(53)/*當日在途加班補休申請時數*/, 
         /*
         *   SSMA warning messages:
         *   O2SS0356: Conversion from NUMBER datatype can cause data loss.
         */
      ...
      BEGIN
         ...
         BEGIN TRY
            /*當日在途加班補休申請時數*/
            SELECT @STOTADDHRS = sum(HRA_OTMSIGN.OTM_HRS)
            FROM HRP.HRA_OTMSIGN
            WHERE 
               ssma_oracle.to_char_date(isnull(HRA_OTMSIGN.START_DATE_TMP, HRA_OTMSIGN.START_DATE), 'yyyy-mm-dd') = isnull(@P_START_DATE_TMP, @P_START_DATE) AND 
...
BEGIN TRY
    EXEC sp_addextendedproperty
        N'MS_SSMA_SOURCE', N'HRP.EHRPHRA12_PKG.hraC010a_add',
        N'SCHEMA', N'hrp',
        N'PROCEDURE', N'EHRPHRA12_PKG$HRAC010A_ADD'
END TRY
BEGIN CATCH

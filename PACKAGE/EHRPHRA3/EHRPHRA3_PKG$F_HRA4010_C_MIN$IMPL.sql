...
USE MIS
GO
IF  EXISTS (SELECT * FROM sys.objects so JOIN sys.schemas sc ON so.schema_id = sc.schema_id WHERE so.name = N'EHRPHRA3_PKG$F_HRA4010_C_MIN$IMPL'  AND sc.name=N'hrp'  AND type in (N'P',N'PC'))
 DROP PROCEDURE [hrp].[EHRPHRA3_PKG$F_HRA4010_C_MIN$IMPL]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE HRP.EHRPHRA3_PKG$F_HRA4010_C_MIN$IMPL  
                           ...
                           SELECT 
                              CASE 
                                 ...
                                 ELSE CAST(fci$3.INV_TM1 AS float(53)) - 
                                    CASE 
                                       WHEN fci$3.HRS > (4 * 60) THEN CAST(fci$3.HRS AS float(53)) - CAST(fci$3.INV_REST AS float(53))
                                       ELSE 
                                          CASE 
                              ...
                              (
                                 SELECT 
                                    ...
                                    ssma_oracle.datediff(
                                       CASE 
                                          WHEN fci$4.REAL_WKOUT < fci$4.REAL_WKIN THEN ssma_oracle.to_date2(ISNULL(ssma_oracle.to_char_date(DATEADD(D, 1, sysdatetime()), 'MMDD'), '') + ISNULL(CAST(fci$4.REAL_WKOUT AS nvarchar(max)), ''), 'MMDD HH24MI')
                                          ELSE ssma_oracle.to_date2(ISNULL(ssma_oracle.to_char_date(sysdatetime(), 'MMDD'), '') + ISNULL(CAST(fci$4.REAL_WKOUT AS nvarchar(max)), ''), 'MMDD HH24MI')
                                       END, ssma_oracle.to_date2(ISNULL(ssma_oracle.to_char_date(sysdatetime(), 'MMDD'), '') + ISNULL(CAST(fci$4.REAL_WKIN AS nvarchar(max)), ''), 'MMDD HH24MI')) * 24 * 60 AS HRS
                                 FROM 
                                          ...
                                          */
                                          (
                                             SELECT 
                                                ...
                                                A.ORG_BY, 
                                                A.ORGAN_TYPE, 
                                                B.WORK_HRS * 60 AS INV_TM1, 
                                                ssma_oracle.datediff(ssma_oracle.to_date2(C.END_REST, 'HH24MI'), ssma_oracle.to_date2(C.START_REST, 'HH24MI')) * 24 * 60 AS INV_REST, 
                                                CASE 
                  ...
                  BEGIN
                     ...
                     BEGIN
                        ...
                        BEGIN TRY
                           ...
                           SELECT @NWORKHRS = HRA_CLASSMST.WORK_HRS * 60
         ...
         */
         DECLARE
             CUR_ABSENCE2 CURSOR LOCAL FOR 
               ...
               SELECT sum(HRA_DAILYTRAN.INSUFFICIENT_MIN) AS ABSENCE, HRA_DAILYTRAN.VAC_DATE
...
BEGIN TRY
    EXEC sp_addextendedproperty
        N'MS_SSMA_SOURCE', N'HRP.EHRPHRA3_PKG.f_hra4010_C_min',
        N'SCHEMA', N'hrp',
        N'PROCEDURE', N'EHRPHRA3_PKG$F_HRA4010_C_MIN$IMPL'
END TRY
BEGIN CATCH

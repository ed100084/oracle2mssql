...
USE MIS
GO
IF  EXISTS (SELECT * FROM sys.objects so JOIN sys.schemas sc ON so.schema_id = sc.schema_id WHERE so.name = N'EHRPHRA3_PKG$F_SET_HRA9610TEMP_TABLE$IMPL'  AND sc.name=N'hrp'  AND type in (N'P',N'PC'))
 DROP PROCEDURE [hrp].[EHRPHRA3_PKG$F_SET_HRA9610TEMP_TABLE$IMPL]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE HRP.EHRPHRA3_PKG$F_SET_HRA9610TEMP_TABLE$IMPL  
...
BEGIN TRY
    EXEC sp_addextendedproperty
        N'MS_SSMA_SOURCE', N'HRP.EHRPHRA3_PKG.f_set_hra9610temp_table',
        N'SCHEMA', N'hrp',
        N'PROCEDURE', N'EHRPHRA3_PKG$F_SET_HRA9610TEMP_TABLE$IMPL'
END TRY
BEGIN CATCH

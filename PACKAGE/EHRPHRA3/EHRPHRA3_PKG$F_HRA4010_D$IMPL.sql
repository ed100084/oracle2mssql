...
USE MIS
GO
IF  EXISTS (SELECT * FROM sys.objects so JOIN sys.schemas sc ON so.schema_id = sc.schema_id WHERE so.name = N'EHRPHRA3_PKG$F_HRA4010_D$IMPL'  AND sc.name=N'hrp'  AND type in (N'P',N'PC'))
 DROP PROCEDURE [hrp].[EHRPHRA3_PKG$F_HRA4010_D$IMPL]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE HRP.EHRPHRA3_PKG$F_HRA4010_D$IMPL  
   ...
   BEGIN
      ...
      BEGIN TRY
         ...
         BEGIN
            ...
            BEGIN TRY
               SELECT @ILATE_DAILYTRAN = count_big(*)
               FROM HRP.HRA_DAILYTRAN
               WHERE 
                  HRA_DAILYTRAN.EMP_NO = @SEMPNO AND 
                  HRA_DAILYTRAN.LATE_FLAG = 'Y' AND 
                  (HRA_DAILYTRAN.VAC_DATE BETWEEN @DSTRARTDATE AND @DENDDATE)
            END TRY
...
BEGIN TRY
    EXEC sp_addextendedproperty
        N'MS_SSMA_SOURCE', N'HRP.EHRPHRA3_PKG.f_hra4010_D',
        N'SCHEMA', N'hrp',
        N'PROCEDURE', N'EHRPHRA3_PKG$F_HRA4010_D$IMPL'
END TRY
BEGIN CATCH

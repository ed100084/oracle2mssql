...
USE MIS
GO
IF  EXISTS (SELECT * FROM sys.objects so JOIN sys.schemas sc ON so.schema_id = sc.schema_id WHERE so.name = N'EHRPHRA3_PKG$F_SET_HRA9610TEMP_DEPTNO$IMPL'  AND sc.name=N'hrp'  AND type in (N'P',N'PC'))
 DROP PROCEDURE [hrp].[EHRPHRA3_PKG$F_SET_HRA9610TEMP_DEPTNO$IMPL]
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE HRP.EHRPHRA3_PKG$F_SET_HRA9610TEMP_DEPTNO$IMPL  
   ...
   BEGIN
               ...
               FROM 
                  (
                     ...
                     WHERE 
                        ...
                        HRA_SUPMST.DEPT_NO = @SDEPTNO
                  )  AS fci
               WHERE ssma_oracle.to_char_date(fci.START_DATE, 'yyyy-mm-dd') BETWEEN @SSTARTDATE AND @SENDDATE
                     ...
                     */
                     ...
                     IF @IWORKMIN > @IREALYWORKMIN
                        BEGIN
                           ...
                           */
                           IF @INSUFFICIENT_TIME > 0.5 AND @LATE_FLAG = 'Y'
                              SET @LATE_FLAG = 'Z'
                                 ...
                                 BEGIN TRY
                                    ...
                                    */
                                    ...
                                    ELSE 
                                       ...
                                       ELSE 
                                          IF ssma_oracle.to_char_date(@SVALIDDATE, 'YYYY-MM-DD') IN (  '2025-01-15' ) AND @SEMP_NO = '114425'
                                             DECLARE
                                                @db_null_statement$6 int
                                          ELSE 
                                             IF ssma_oracle.to_char_date(@SVALIDDATE, 'YYYY-MM-DD') IN (  '2025-05-20' ) AND @SEMP_NO = '115083'
                                                DECLARE
      ...
      BEGIN CATCH
         BEGIN
            ...
            SET @return_value_argument = CAST(@SEMP_NO AS numeric(38, 10))
...
BEGIN TRY
    EXEC sp_addextendedproperty
        N'MS_SSMA_SOURCE', N'HRP.EHRPHRA3_PKG.f_set_hra9610temp_deptno',
        N'SCHEMA', N'hrp',
        N'PROCEDURE', N'EHRPHRA3_PKG$F_SET_HRA9610TEMP_DEPTNO$IMPL'
END TRY
BEGIN CATCH
